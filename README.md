...
| `JEST_JUNIT_UNIQUE_OUTPUT_NAME` | `uniqueOutputName` | Create unique file name for the output leveraging the `outputName` as a prefix if given `${outputName}.xml` or a default of `junit.xml` if `outputName` is not specified, overrides `outputName` | `false` | N/A
